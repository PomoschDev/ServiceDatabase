...
func (s *serverAPI) CreateUser(ctx context.Context, req *CreateUserRequest) (*CreateUserResponse, error) {
	...
	/*
		err = utilities.Transformation(findUser.Card, response.Card)
		if err != nil {
			logger.Log.Error("utilities.Transformation(findUser.Card, response.Card)", sl.Err(err))
			return nil, status.Error(codes.Internal, fmt.Sprintf("Ошибка на стороне сервиса: %v", err))
		}

		err = utilities.Transformation(findUser.Company, response.Company)
		if err != nil {
			logger.Log.Error("utilities.Transformation(findUser.Company, response.Company)", sl.Err(err))
			return nil, status.Error(codes.Internal, fmt.Sprintf("Ошибка на стороне сервиса: %v", err))
		}

		err = utilities.Transformation(findUser.Donations, response.Donations)
		if err != nil {
			logger.Log.Error("utilities.Transformation(findUser.Donations, response.Donations)", sl.Err(err))
			return nil, status.Error(codes.Internal, fmt.Sprintf("Ошибка на стороне сервиса: %v", err))
		}
	*/
...
func (s *serverAPI) UpdateUser(ctx context.Context, req *UpdateUserRequest) (*HTTPCodes, error) {
	user := &models.User{ID: req.GetId()}
	if err := user.DeleteByID(); err != nil {
		logger.Log.Error("user.DeleteByID()", sl.Err(err))
		return nil, status.Error(codes.Internal, fmt.Sprintf("Ошибка на стороне сервиса: %v", err))
